...
function main(req, res, parts, respond) {
  ...
  case 'DELETE':
    ...
    break;
  case 'PATCH':
    markCompleted(req, res, parts[0], respond);
    break;
  default:
...
function markCompleted(req, res, id, respond) {
...
function sendItem(req, res, id, respond) {
  ...
  if (Array.isArray(list) && list[0] === null) {
    ...
    tran = transitions("markCompletedLink");
    tran.href = root + "/" + id +"?=markcompleted";
    tran.rel = ["markcompleted","item"];
    coll.splice(coll.length, 0, tran);

    tran = transitions("clearCompletedLink");
    tran.href = root + "/" + id +"?clearcompleted";
    tran.rel = ["clearcompleted","item"];
    coll.splice(coll.length, 0, tran);
