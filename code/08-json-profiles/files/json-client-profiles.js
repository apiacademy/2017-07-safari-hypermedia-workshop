...
function json() {
  ...
  function items() {
    ...
    if(g.msg.todo) {
      ...
      for(var item of coll) {
        ...
        for(var f in item) {
          if(f!=="link") {
            p = d.data({className:"item "+f, text:item[f].prompt, value:item[f].value+"&nbsp;"});
            d.push(p,dd);
          }
        }
  ...
  function itemActions(dt, item, single) {
    ...
    a = d.anchor({
      href:item.link.href||"#",  
      rel:item.link.rel.join(" ")|"item",
      className:"item action",
      text:item.link.prompt||"Item"
    });
    a.onclick = httpGet;
    d.push(a,dt);
  ...
  function jsonForm(e) {
    ...
    for(var prop in coll) {
      ...
      if(g.msg.todo[0][prop]) {
        val = val.replace("{"+prop+"}",g.msg.todo[0][prop].value);
      } 
      p = d.input({
